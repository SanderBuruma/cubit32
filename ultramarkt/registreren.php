...
if (isset($_POST['submit'])){
  ...
  $telefoonNr = filter_var($_POST['telefoonNr'],FILTER_SANITIZE_STRING);
  ...
  if       ($resultnruser > 0){
    ...
    $_SESSION['warning'] = "geen paswoord";

	} elseif (empty($telefoonNr)){

    $_SESSION['warning'] = "geen telefoonnr";

	} elseif (empty($passwordConfirm)){

    $_SESSION['warning'] = "geen herhaald paswoord";

	} elseif (empty($username)){

    $_SESSION['warning'] = "geen gebruikersnaam";

	} elseif (empty($email)){

    $_SESSION['warning'] = "geen E-Mail";
...
<form id="registreer" action="registreren.php" method="POST">
  ...
  <input type="text" name="username" pattern="[A-Za-z0-9 \-\_]{6,}" oninvalid="setCustomValidity('minstens 6 tekens en alleen letters, nummers, `-` en `_`')" placeholder="gebruiker"><br/>
  ...
  <input type="text" name="telefoonNr" pattern="\d{2,5}-\d{4,15}"  oninvalid="setCustomValidity('alleen nrs en \'-\' toegestaan')" placeholder="telefoon nummer ex. 06-12345678"><br/>
