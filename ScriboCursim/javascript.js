...
function wpmCounterChange(){
	document.getElementById("wpm-counter").innerHTML = Math.round(document.getElementById("text-input").value.length*12/((+new Date() - startTypingDate)/1e3)) + ' wpm  ' + Math.floor(((+new Date() - startTypingDate)/1e3)/60)+"m "+(Math.floor((+new Date() - startTypingDate)/1e3)%60)+'s';
}

function mainMenuPractice(a){//trigger when >practice< is clicked
	selectCategory = document.getElementById('practice-category-select').selectedOptions[0].value;
	xmlhttp.open("GET","getrandomtext.php?category="+selectCategory);
	xmlhttp.send();
...
xmlhttp.onreadystatechange = function() {
	if (this.readyState == 4 && this.status == 200) {//start the practice
		let textInformation = this.responseText.split("%SPLIT%");
		currentText = textInformation[2]+' ';
		...
		document.getElementById('race-info-source').innerHTML=  textInformation[1];
		document.getElementById('race-info-book').innerHTML=    textInformation[0];
	}
}