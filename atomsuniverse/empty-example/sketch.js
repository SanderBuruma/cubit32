...
function draw(){
  //refill with atoms after merging them
  ...
  for (let i=0;i<atoms.length;i++){ //check all atoms against each other through iteration // this has n^2 complexity
    ...
    //check all atoms against each other and calculate gravity
    for (let j=0;j<atoms.length;j++){
      ...
      if (i!==j && !atoms[j].collision && !atoms[i].collision){
        ...
        if (distance*2<atom2Radius+atom1Radius){
          
          //for collision merge atoms by averaging out velocities, positions, etc
          ...
          atoms[j].collision = true; //note: No more than 1 collision per frame can be processed
          collidingAtoms++;
          debugLog += `Atom ${j} colliding with ${i}. `;
        }
        let distanceX = atoms[i].pos[0]-atoms[j].pos[0];
        let distanceY = atoms[i].pos[1]-atoms[j].pos[1];
        acceleration = (atoms[j].neutrons*G) / Math.pow(distance,2) - /*GM/r^2 = m/s^2*/ (Math.pow/*this is a random fudge factor*/(atoms[j].protons,atomProtonExp)*G2)/Math.pow(distance,4) /*similar to above but r^4*/;
